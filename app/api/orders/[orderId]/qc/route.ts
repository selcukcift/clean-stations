...
export async function POST(
  ...
  try {
    ...
    const qcResult = await prisma.$transaction(async (tx) => {
      ...
      // Create new item results with media attachments
      if (validatedData.itemResults.length > 0) {
        for (const item of validatedData.itemResults) {
          const itemResult = await tx.orderQcItemResult.create({
            data: {
          ...
          if (item.mediaFileIds && item.mediaFileIds.length > 0) {
            await tx.qcItemResultMediaAttachment.createMany({
              data: item.mediaFileIds.map((fileId, index) => ({
                orderQcItemResultId: itemResult.id,
                fileUploadId: fileId,
                mediaType: 'photo', // Default to photo, can be enhanced later
                caption: null
              }))
            });
          }
