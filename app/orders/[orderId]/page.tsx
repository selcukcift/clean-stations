...
import { ConfigurationDisplayRedesigned } from "@/components/order/shared/ConfigurationDisplayRedesigned"
...
export default function OrderDetailsPage() {
  ...
  const handleEditConfiguration = async (buildNumber: string) => {
    if (!user || user.role !== 'ADMIN') {
      toast({
        title: "Access Denied",
        description: "Only administrators can edit configurations.",
        variant: "destructive"
      })
      return
    }

    // Check if order can be edited (only ORDER_CREATED status allows editing)
    if (order?.orderStatus !== 'ORDER_CREATED') {
      toast({
        title: "Cannot Edit",
        description: `Orders in ${order?.orderStatus} status cannot be edited. Only orders in ORDER_CREATED status can be modified.`,
        variant: "destructive"
      })
      return
    }
    ...
    toast({
      title: "Redirecting",
      description: `Opening configuration editor for build ${buildNumber}...`,
    })

    // Navigate to the order edit page with build number focus
    router.push(`/orders/edit/${orderId}?focusBuild=${buildNumber}`)
  }

  const handleDeleteConfiguration = async (buildNumber: string) => {
    if (!user || user.role !== 'ADMIN') {
      toast({
        title: "Access Denied", 
        description: "Only administrators can delete configurations.",
        variant: "destructive"
      })
      return
    }
    ...
    const confirmed = window.confirm(
      `Are you sure you want to delete the configuration for build ${buildNumber}?\n\nThis action cannot be undone and will:\n• Remove the configuration from this order\n• Update the BOM\n• Potentially affect manufacturing\n\nContinue?`
    )

    if (!confirmed) return

    try {
      const response = await nextJsApiClient.delete(`/orders/${orderId}/configurations/${buildNumber}`)
      
      if (response.data.success) {
        toast({
          title: "Configuration Deleted",
          description: `Build ${buildNumber} configuration has been removed from the order.`,
        })
      ...
      toast({
        title: "Delete Failed",
        description: error.response?.data?.message || "Failed to delete configuration. Please try again.",
        variant: "destructive"
      })
    }
  }
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="overview" className="space-y-3">
        ...
        <TabsContent value="configuration" className="space-y-4">
          <ConfigurationDisplayRedesigned
